...
program g_minimum_distance
  ...
  real, allocatable :: md_count2(:), md_frame_count2(:)
  real :: md_sum
  real :: md_sum_random
  ...
  allocate( gmd(nbins), kb(nbins), &
            ...
            shellvolume(nbins), &
            md_count2(nbins), md_frame_count2(nbins) )
  ...
  do icycle = 1, ncycles 
    ...
    do kframe = 1, nfrcycle
      ...
      do irad = 1, nbins
        md_frame_count2(irad) = 0
      end do
      do i = 1, nrsolvent
        irad = int(float(nbins)*mind_mol(i)/cutoff)+1
        if( irad <= nbins ) then
          md_frame_count2(irad) = md_frame_count2(irad) + 1.e0
      ...
      do irad = 1, nbins
        md_count2(irad) = md_count2(irad) + md_frame_count2(irad)**2
      end do
  ...
  do i = 1, nbins
    ...
    md_count2(i) = md_count2(i)/frames
  ...
  write(20,"('#')") 
  write(20,"('#   1-DISTANCE         2-GMD      3-KB INT    4-MD COUNT  5-COUNT RAND      6-SUM MD"//&
             "    7-SUM RAND   8-SHELL VOL 9-MD COUNT2')")
  ...
  do i = 1, nbins
    ...
    lineformat = trim(adjustl(lineformat))//",tr2,"//format(md_count2(i))
